...
//	}
    ...
    public static AccessToken getNewAccessToken(){//String appid, String appsecret
        ...
        //调用接口，获取json字符串
        String jsonString = WX_CommonUtil.httpsRequest(requestUrl, "GET", null);
        //将json字符串转换成java对象
        {
...
//			System.out.println(strTemp[0]);//****************
            token.setAccessToken(strTemp[0]);//获取access_token

            json = strTemp[1];
            strTemp = json.split("\":");
            json = strTemp[1].substring(0, strTemp[1].length()-1);
//			System.out.println(Integer.parseInt(json));//*************
            token.setExpiresin(Integer.parseInt(json));//获取expires_in
        }
    ...
    public static AccessToken json_To_Token(String json){
